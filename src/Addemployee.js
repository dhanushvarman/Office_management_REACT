...
function Addemployee() {
	...
	const formik = useFormik({
		...
		validate: (values) => {
			...
			if(!values.date){
				error.data = "Please select joining Date"
			}
		...
		onSubmit: async (values) => {
			...
			try {
				setloading(true)
				const data = await axios.post(`${config.api}/employee/create`, values)
				console.log(data)
				setloading(false)
	...
	return (
		<div class="modal fade"	 tabIndex="-1" role="dialog" id="addEmployeeModal" aria-labelledby="addEmployeeModal"
			aria-hidden="true">
                        ...
                        ${formik.touched.age && !formik.errors.age ? 'success-box' : ''}`}
								...
								<div class="form-group col-md-6">
									...
									<input type="text"
										...
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
										className={`form-control ${formik.errors.designation ? 'error-box' : ''} 
                        ${formik.touched.designation && !formik.errors.designation ? 'success-box' : ''}`}
										id="" placeholder="Enter Designation" />
										{
										formik.errors.designation ? <span style={{ color: "red" }}>{formik.errors.designation}</span> : null
									}
								</div>
								<div class="form-group col-md-6">
                        ...
                        ${formik.touched.department && !formik.errors.department ? 'success-box' : ''}`}
								...
								<div class="form-group col-md-6">
									...
									<input type="date"
										...
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
										className={`form-control ${formik.errors.date ? 'error-box' : ''} 
                        ${formik.touched.date && !formik.errors.date ? 'success-box' : ''}`} 
										id="" placeholder="Select a date..." />
										{
										formik.errors.date ? <span style={{ color: "red" }}>{formik.errors.date}</span> : null
									}
								</div>
							</div>
